...
# 数据库初始化脚本 - 使用 Docker exec
# 直接在 Docker 容器内部执行 MySQL 命令，不需要本地安装 MySQL 客户端
...
echo -e "${BLUE}🔧 使用 Docker exec 初始化数据库...${NC}"
echo ""
...
echo "   Container: warehouse-mysql-dev"
...
if docker exec ${CONTAINER_NAME} mysql -u ${DB_USER} -p${DB_PASSWORD} ${DB_NAME} -e "SELECT 1;" > /dev/null 2>&1; then
    ...
    echo "   1. MySQL 容器正在运行"
    echo "   2. 等待容器完全启动（MySQL 需要一些时间初始化）"
...
if [ $SQL_EXIT_CODE -eq 0 ]; then
    ...
    docker exec ${CONTAINER_NAME} mysql -u ${DB_USER} -p${DB_PASSWORD} ${DB_NAME} -e "SHOW TABLES;" 2>/dev/null || echo "   (表列表获取失败)"
    echo ""
else
    echo ""
