...
async def fetch(
    ...
    try:
        ...
        if cabinet_id is not None:
            ...
            response_data = CabinetResponse.model_validate(cabinet).model_dump(
                mode="json",
                exclude_none=True,
            )
            return success_response(data=response_data)
        
        # 如果同時帶入 home_id 和 room_id，返回該家庭該房間的所有櫥櫃
        if home_id is not None and room_id is not None:
            cabinets = await _get_db_cabinets_by_home_and_room(home_id, room_id, db)
            cabinets_data = [
                CabinetResponse.model_validate(cabinet).model_dump(
                    mode="json",
                    exclude_none=True,
                )
        ...
        # 如果只帶入 home_id，返回該家庭的所有櫥櫃
        if home_id is not None:
            cabinets = await _get_db_cabinets_by_home(home_id, db)
            cabinets_data = [
                CabinetResponse.model_validate(cabinet).model_dump(
                    mode="json",
                    exclude_none=True,
...
async def _get_db_cabinets_by_home(
    ...
    result = await db.execute(
        select(Cabinet).where(Cabinet.home_id == home_id)
    )
    return list(result.scalars().all())
